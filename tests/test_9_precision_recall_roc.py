...
@pytest.mark.parametrize(
    "roc_curve_gt_file_name, roc_curve_gt_file_link, roc_curve_file_name, "
...
def test_check_created_roc_precision_curve_file(
    roc_curve_gt_file_name,
    roc_curve_gt_file_link,
    roc_curve_file_name,
    precision_recall_gt_file_name,
    precision_recall_gt_file_link,
    precision_recall_file_name,
    min_error,
    tagging_csv_file_name,
    precision_recall_link_pkl,
    precision_recall_gt_pkl_file_name,
):
    ...
    if not os.path.exists(gt_pkl_path):
        try:
            ...
            pytest.fail(
                f"gdown failed to download {precision_recall_link_pkl}. Error: {e}"
            )

    y_true = [1, 1, 1, 0, 1, 1, 0, 1, 0, 0, 0, 0]
    y_predict = (
        (100 * np.arange(1, -1 / (len(y_true) - 1), -1 / (len(y_true) - 1))).astype(
            np.int32
        )
    ).astype(np.float32) / 100

    recall1, precision1, _, _, _, _ = generate_precision_recall_auc_graphs(
        y_true, y_predict, version="v1", show_threshold=False
    )
    y_true[3] = 1
    recall2, precision2, _, _, _, _ = generate_precision_recall_auc_graphs(
    ...
    assert os.path.exists(output_roc_path), (
        f"ROC curve image was not created at {output_roc_path}"
    )
    assert os.path.exists(output_pr_path), (
        f"Precision-recall curve image was not created at {output_pr_path}"
    )
